...
public class CreateAnAccountWebController {
    ...
    public void createAccount(){
        try{
            Customer customer = generateCustomer(SPANISH_CODE_LANGUAGE, COUNTRY_CODE, EMAIL_DOMAIN);
    ...
    public void goTheHome() {
        try {
            CreateAnAccountPage createAnAccountPage = new CreateAnAccountPage(webAction.getDriver());
            webAction.click(createAnAccountPage.getHome(), true);
        }catch (WebActionsException e){
            Report.reportFailure("No es posible ir al home de la pagina", e);
        }
